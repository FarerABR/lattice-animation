...
class LatticeFirstScene(Scene):
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, points,
                  shift=DOWN, lag_ratio=0.02), run_time=0.8)
        self.wait(2)
        ...
        self.play(Transform(basis1, basis3), Transform(label1, label3),
                  Transform(basis2, basis4), Transform(label2, label4), *animations, run_time=1)
        self.wait(2)
        ...
        self.play(Transform(basis1, Arrow(ORIGIN, v1, buff=0, color=BLUE)),
                  Transform(basis2, Arrow(ORIGIN, v2, buff=0, color=YELLOW)),
                  Transform(label1, MathTex("\mathbf{v}_1").next_to(v1, DOWN)),
                  Transform(label2, MathTex("\mathbf{v}_2").next_to(v2, RIGHT)), *ani2, run_time=1)
        self.wait(1)

        self.play(FadeOut(basis1), FadeOut(basis2), FadeOut(
            label1), FadeOut(label2), FadeOut(points))
        ...
        self.play(FadeIn(plane),
                  FadeIn(dt),
                  Write(lbl), run_time=0.5)
        self.wait(1)

        ar11 = Arrow(ORIGIN,
                     np.array([1, 0, 0]), buff=0, color=BLUE)
        ar22 = Arrow(ar11.end,
                     np.array([1, 1, 0]), buff=0, color=YELLOW)
        ar33 = Arrow(ar22.end,
                     np.array([1, 2, 0]), buff=0, color=YELLOW)
        ar44 = Arrow(ar33.end,
                     np.array([1, 3, 0]), buff=0, color=YELLOW)
        ...
        self.play(FadeOut(ar11), FadeOut(ar22),
                  FadeOut(ar33), FadeOut(ar44))
        ...
        self.play(FadeOut(basis3), FadeOut(basis4), FadeOut(
            label3), FadeOut(label4))

        self.play(FadeIn(ar11), FadeIn(ar22),
                  FadeIn(ar33), FadeIn(ar44))

        ar1 = Arrow(ORIGIN, -v3, buff=0, color=RED)
        ar2 = Arrow(ar1.end,
                    np.array([-6, -2, 0]), buff=0, color=RED)
        ar3 = Arrow(ar2.end,
                    np.array([-1, 1, 0]), buff=0, color=GREEN)
        ar4 = Arrow(ar3.end,
                    np.array([4, 4, 0]), buff=0, color=GREEN)
        ar5 = Arrow(ar4.end,
                    np.array([1, 3, 0]), buff=0, color=RED)
        ...
        self.play(FadeOut(ar1), FadeOut(ar2),
                  FadeOut(ar3), FadeOut(ar4),
                  FadeOut(ar5), FadeOut(ar11),
                  FadeOut(ar22), FadeOut(ar33),
                  FadeOut(ar44), FadeOut(dt),
                  FadeOut(lbl), FadeOut(plane))

        dt2 = Dot(point=np.array([-2, 1, 0]), radius=0.06, color=YELLOW_E)
        dt3 = Dot(point=np.array([-2.56, 1.475, 0]), radius=0.06, color=BLUE)
        self.play(GrowArrow(basis1), Write(label1),
                  GrowArrow(basis2), Write(label2))

        self.play(FadeIn(dt2))
        self.wait(1)
        self.play(FadeIn(dt3))
        self.wait(2)
        self.play(FadeOut(basis1), FadeOut(basis2),
        ...
        lbl3 = MathTex(
            "(-2.56,1.475)").next_to(np.array([-2.56, 1.475, 0]), UP)
        lbl4 = MathTex("(-3,1)").next_to(np.array([-3, 1, 0]), LEFT)
        self.play(FadeIn(dt4))
        self.play(Write(lbl2), Write(lbl3),
                  Write(lbl4), run_time=0.5)
